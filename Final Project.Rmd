...
---
## Introduction
We want to compare two data tables to see how the differents sets correlate with each other. For this specific case we want to compare how the overall stats for a professional soccer player in the video game FIFA compares with their real life estimated transfer market. The purpose of this is to show how we can use R to alter data tables and show correlations between two datasets that may not be obvious just by looking at them. 

## Libraries Used
When using R we need to implement some library packages to make our lives easier when doing specialized things such as, string functions, html scraping, and data frame implementation. 
...
```
## Useful links
Soccer Transfer Market Site: https://www.transfermarkt.com/spieler-statistik/wertvollstespieler/marktwertetop

Fifa 19 Dataset: 
https://www.kaggle.com/karangadiya/fifa19

R Regular Expressions: https://rstudio-pubs-static.s3.amazonaws.com/74603_76cd14d5983f47408fdf0b323550b846.html
...
value_tab <- read_html(url) %>%
  ...
  set_colnames(c("rank", "player", "na", "Name", "position", "age", "a", "b", "market_value")) %>%
  as_tibble() 
...
value_tab <- value_tab %>%
  select(rank, Name, position, age, market_value) %>%
...
value_tab$market_value <- gsub("([0-9]+),([0-9]{2}) Mill. \200", "\\1\\20000", value_tab$market_value)

# Turn the market value attribute into a numeric double
value_tab$market_value <- as.numeric(value_tab$market_value)
...
fifa <- read.csv("data.csv", encoding = "UTF-8") %>%
  slice(0:1000)
...
fifa$Name <- gsub("([A-Z])\\S+ (.+)", "\\1. \\2", fifa$Name)

# Changing some outliers in the data
fifa$Name[fifa$Name == "N. Jr"] = "Neymar"
fifa$Name[fifa$Name == "D. Gea"] = "D. de Gea"
fifa$Name[fifa$Name == "K. D. Bruyne"] = "K. De Bruyne"

fifa %>%
  select(X.U.FEFF.Row ,Name, Overall, Potential, Club) %>%
  slice(1:50)
```

## Joining Two Data Tables Together
Here, all of our hard work finally pays off. We use an inner join from the dplyr library to join the transfer market table and the FIFA table together. We join them together by the "Name" attribute. We use an inner_join() here because we want all of the columns from the transfer market table and the FIFA table. We can still use inner_join() to join together names so that all of their attributes appear under one row. 
```{r}
# Join the two tables together by name
value_tab %>%
  inner_join(fifa, by="Name")
```
